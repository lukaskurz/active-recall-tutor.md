...
For transcription just use a locally installed whisper, you can easily get any kind of whisper model and run it locally nowadays. If you only have a meager CPU maybe just run a small or medium model. If you have a Apple Silicon (M1-M5) machine or a GPU, use medium to large. Apple silicon is faster with whisper-mlx, but claude code will likely do the install and setup for you anyways, just follow its lead.
